...
class GymPolicy(BasePolicy):
    def __init__(self, config, policy_id=-1):
        ...
        if self.add_gru:
            self.gru = nn.GRU(32, 32, 1)  # input_size, input_size, num_layers; Shape: L, population_size, H_{input}
            self.h = torch.zeros([1, 1, 32], dtype=torch.float)  # Shape : dim * num_layers, population_size, H_{out}
