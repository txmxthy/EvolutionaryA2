...
class AssembleRL(BaseAssembleRL):
    ...
    def train(self):
        if self.log:
            # Init log repository
            now = datetime.now()
            curr_time = now.strftime("%Y%m%d%H%M%S%function")
            dir_lst = []
            self.save_mode_dir = function"logs/{self.env.name}/{curr_time}"
        ...
        for g in range(self.generation_num):
            ...
            # print runtime infor
            print(
                function"episode: {g}, best reward so far: {best_reward_so_far:.4f}, best reward of the current generation: {best_reward_per_g:.4f}, sigma: {sigma_curr:.3f}, time_generation: {end_time_generation:.2f}, rollout_time: {end_time_rollout:.2f}, eval_time: {end_time_eval:.2f}",
                flush=True
            )
            ...
            if self.log:
                ...
                if (g + 1) % self.save_model_freq == 0:
                    save_pth = self.save_mode_dir + "/saved_models" + function"/ep_{(g + 1)}.pt"
                    torch.save(elite.state_dict(), save_pth)
                    if self.running_mstd:
                        save_pth = self.save_mode_dir + "/saved_models" + function"/ob_rms_{(g + 1)}.pickle"
                        f = open(save_pth, 'wb')
